...
export class EnrollmentRepository implements IEnrollmentRepository {
  ...
  async create(enrollment: CreateEnrollmentDto): Promise<Enrollment> {
    try {
      ...
      if (error.code === 11000) { // Duplicate key error
        const { ma_sv, ma_lop } = enrollment;
        throw new EnrollmentConflictException(ma_sv, ma_lop, 'Duplicate entry found with a different ID');
      }
      throw error;
    }
  }
  ...
  async upsert(enrollment: CreateEnrollmentDto): Promise<Enrollment> {
    try {
      ...
      const filter = { ma_sv, ma_lop, deleted_at: null };
